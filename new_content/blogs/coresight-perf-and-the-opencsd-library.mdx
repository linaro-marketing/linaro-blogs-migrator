...
To bridge the gap between the CoreSight framework and the Perf core, CoreSight tracers (ETMv3/4 and PTM) are modelled as PMUs.  At boot time the newly introduced function [*etm\_perf\_init()*](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/drivers/hwtracing/coresight/coresight-etm-perf.c?id=refs/tags/v4.7-rc1#n370) registers an *etm\_pmu* with the perf core:

*#define CORESIGHT\_ETM\_PMU\_NAME “cs\_etm”*

*static struct pmu etm\_pmu;*
...
Calling *perf\_pmu\_register() \_creates a new PMU with the characteristics found in the \_struct pmu* given as a parameter.  When a successful registration has completed the new PMU can be found alongside the other PMUs catalogued at boot time:
...
Among other things, functions provided to the *struct auxtrace\_record* deal with how to find tracer specific metadata, the presentation and formatting of the metadata in the *perf.data* file along with specifics related to the size and mapping of the ring buffer shared between the kernel and user space.  That ring buffer is then used to retrieve trace data from the kernel.
